  ...
  return (
    <div style={{ gridColumn: "1 / -1", background: C.paper2, border: `1.5px solid ${C.ink}`, borderRadius: 4, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <button onClick={onCancel} style={{ background: "transparent", border: `1.5px solid ${C.inkSoft}`, color: C.inkSoft, borderRadius: 3, padding: "9px 18px", cursor: "pointer", fontFamily: FD }}>ยกเลิก</button>
        <button onClick={onAdd} disabled={adding} style={{ background: C.sage, color: C.paper, border: "none", borderRadius: 3, padding: "9px 18px", fontWeight: 700, cursor: "pointer", fontFamily: FD }}>{adding ? (imgUploading ? "กำลังอัปโหลดรูป..." : "กำลังเพิ่ม...") : "บันทึก"}</button>
      </div>
    </div>
  );
}
